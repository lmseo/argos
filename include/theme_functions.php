 ...
 * @returns an array of values (width, height, column class) to be used in the template 
*/
function lmseo_portfolio_items_values( $columns ){
	$values = array();
	if(  $columns == 2  )
	{
		$values['size'] = '2col';
		$values['class'] = '-2col';
		$values['fz_class'] = 'small-12 medium-6';
	}
	...
	if(  $columns == 3  )
	{
		$values['size'] = '3col';
		$values['class'] = '-3col';
		$values['fz_class'] = 'small-12 medium-6 large-4';
	}
	...
	if(  $columns == 4  )
	{
		$values['size'] = '4col';
		$values['class'] = '-4col';
		$values['fz_class'] = 'small-12 medium-4 large-3';
	}
...
function lmseo_portfolio_template(  $num_items, $type ){
	...
	?>
    <section class="zp-grid-wrapper">
				<div class="<?php echo zp_portfolio_style_effects( genesis_get_option( 'zp_portfolio_animation' , ZP_SETTINGS_FIELD ) ); ?>"><div class="portfolio_archive_element_wrapper wrap medium-collapse">
	...
	if(  have_posts( ) ) {											
 		while (  have_posts(  )  ) {
			...
			/** generate the final item HTML */
			/*$html.= '<div class="element '.$class.''.$_values['class'].' "><a href="'.$link.'" title="'.$t.'" '.$rel.'><span class="portfolio_overlay"><h4 class="portfolio_title">'.$t.'</h4></span>'.get_the_post_thumbnail( $post->ID, $_values['size'] , array('alt' => $t) ).'</a></div>';*/
			/*This if will add end class to the last image of the array per Foundation specifications*/
			$last_post_class="";
