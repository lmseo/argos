...
function zp_slider(  $atts, $content = null  ){
	...
	$output ='';
	$mobile_otput ='<div class="slider_shortcode_mobile show-for-small-only">';
	$output .= '<div class="slider_shortcode hide-for-small-only" style="height: '.$height.'; width:'.$width.';"><div class="flexslider '.$name. '"><ul class="slides">';
	...
	while($recent->have_posts()) : 
		...
		$imageAtt = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), $size );
		$images =$imageAtt[0];
		//print_r($images);
		...
		if($type == "youtube"){
			...
			$output.= '<li style="background-image: url('.$images.'); height: '.$height.'">';
				...
				if( $content ) {
					$mobile_otput .=  '<div class="excerpt">'.$content.'</div>';
					$output .=  '<div class="excerpt">'.$content.'</div>';
				}
				if( $button ) {
					$mobile_otput .= '<div  class="mobile_slider_button_wrapper text-center"><a href="'.$link.'" class="button" >'.$button.'</a></div>';
